...
export function UserDetailDrawer({
  ...
  const handleImpersonate = async () => {
    ...
    if (token) {
      try {
        ...
        toast.success(zh ? "模拟登录令牌已复制（1 小时内有效）" : "Impersonation token copied (valid 1h)");
      } catch {
        toast.success(zh ? "模拟登录令牌已创建（1 小时内有效）" : "Impersonation token created (1h)");
      }
    }
  };
  ...
  const handleCopyId = async () => {
    try {
      ...
      toast.success(zh ? "用户 ID 已复制" : "User ID copied");
    } catch {
      ...
      >
        ...
        <div className="flex-1 scrollbar-thin overflow-y-auto px-6 py-5">
          <div className="flex flex-wrap gap-1.5">
            <Badge variant={user.is_active ? "default" : "secondary"} className="text-[10px]">
              {user.is_active ? (zh ? "正常" : "Active") : (zh ? "已停用" : "Suspended")}
            </Badge>
          ...
          <section className="mt-7">
            <h3 className="text-foreground/55 mb-3 font-mono text-[11px] tracking-wider uppercase">
              {zh ? "最近会话" : "Recent conversations"}
            </h3>
            {convsLoading ? (
              <LoadingState variant="skeleton-list" rows={3} />
            ) : !conversations || conversations.length === 0 ? (
              <p className="text-foreground/55 text-xs">{zh ? "暂无会话。" : "No conversations."}</p>
            ) : (
                ...
                {conversations.map((c) => (
                  ...
                  >
                    ...
                    <a
                      ...
                      title={zh ? "打开会话" : "Open conversation"}
                    >
        ...
        <footer className="border-foreground/10 flex flex-wrap items-center gap-2 border-t px-6 py-4">
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>{zh ? `删除 ${user.email}？` : `Delete ${user.email}?`}</AlertDialogTitle>
                <AlertDialogDescription>
                  {zh ? "这将永久删除该用户及其会话和额度余额，且无法撤销。" : "Permanently removes the user, their conversations, and credit balance. This can't be undone."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
