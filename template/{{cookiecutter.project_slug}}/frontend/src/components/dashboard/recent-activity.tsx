...
export function RecentActivity({ limit = 6 }: { limit?: number }) {
  ...
  const load = async () => {
    ...
    try {
        ...
        for (const tx of txResp.value.items.slice(0, 4)) {
          ...
          events.push({
            ...
            title:
              tx.description ||
              (isPositive
                ? `+${t("creditsValue", { count: tx.delta.toLocaleString() })}`
                : t("creditsValue", { count: tx.delta.toLocaleString() })),
            description: humanizeTxType(tx.type, locale),
      ...
      setError(getErrorMessage(err, t("loadActivityFailed")));
    }
  };
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-foreground/55 truncate text-xs">
          {item.description}
          {item.description && " · "}
          {timeAgo(item.timestamp, locale === "zh" ? "zh-CN" : locale)}
        </p>
      </div>
    </div>
  );
...
function humanizeTxType(value: string, locale: string): string {
  const labels: Record<string, string> = locale === "zh"
    ? { grant_trial: "试用额度", subscription_renewal: "订阅续费", usage: "用量扣费", top_up: "额度充值" }
    : {};
