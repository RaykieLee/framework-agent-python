...
framework-agent-python upgrade [--path DIR] [--to VERSION] [--dry-run] [--with-new-features] [--force]
framework-agent-python upgrade finalize [--path DIR]
framework-agent-python upgrade recover  [--path DIR]
```
...
**"No `.framework-agent-python.json` found — run recovery first."**
...
.framework-agent-python.json.pending`。保留 `-f` 和 `rm` —— 见[随时撤销](#sui-shi-che-xiao)。
